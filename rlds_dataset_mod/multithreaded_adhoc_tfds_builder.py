...
class MultiThreadedAdhocDatasetBuilder(tfds.core.dataset_builders.AdhocBuilder):
    ...
    def __init__(
        self, *args, generator_fcn, n_workers, max_episodes_in_memory, train_file_ids,new_dataset_size, **kwargs
    ):
        super().__init__(*args, **kwargs)
        self._generator_fcn = generator_fcn
        self._n_workers = n_workers
        self._max_episodes_in_memory = max_episodes_in_memory
        self.train_file_ids = train_file_ids,  # 保存处理后的编号的列表
    ...
    def _download_and_prepare(  # pytype: disable=signature-mismatch  # overriding-parameter-type-checks
        ...
        split_builder = ParallelSplitBuilder(
            ...
            dataset_size=self.new_dataset_size,
            ...
            train_file_ids=self.train_file_ids,
        )
...
def parse_examples_from_generator(
    ...
    print(split_name + f"[{episodes[0]}:{upper_str}]")

    # 修改逻辑，直接传入episode列表
    # print(f"episodes: {episodes}")
    # split_parts = []
    # for episode in episodes:
    #     split_parts.append(f"{split_name}[{episode}:{episode+1}]")
    # split = '+'.join(split_parts)
    # print(split)
    # generator = fcn(split=split)
    # print('suceess')
    # print(generator)
    outputs = []
...
class ParallelSplitBuilder(split_builder_lib.SplitBuilder):
    def __init__(
        self, *args, generator_fcn, n_workers, max_episodes_in_memory, train_file_ids, **kwargs
    ):
        super().__init__(*args, **kwargs)
        self._generator_fcn = generator_fcn
        self._n_workers = n_workers
        self._max_episodes_in_memory = max_episodes_in_memory
        self.train_file_ids = train_file_ids[0],  # 保存处理后的编号的列表,被包了两层
